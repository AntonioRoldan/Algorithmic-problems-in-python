...
def iterative_build_pile(cubes, cubes_by_colour, colours):
    ...
    cubes_by_colour, colours = arrange_cubes_by_colour(
        cubes)  # We get a dictionary with colours as keys and cubes' names and lengths as values
    first_block = True
    while (len(colours) > 1):  # As we place one block upon the other we will eventually have a single colour left
        ...
        if (first_block):
            ...
            cur_colour = cubes[max_block[0]][0]
        delete_cube(cubes_by_colour, max_block)
        delete_colour_if_its_over(cubes_by_colour, cur_colour)  # We delete a certain colour if we are already out of that colour
        pile_of_blocks.append(max_block)
    return tuple(reversed(pile_of_blocks))
    # If we have already chosen the maximum element from the red cubes and the next cubes will have a larger length
    # it will never be the case that any resulting cube of a given last colour can be placed on top of the pile since all previously placed cubes have larger length
    # and so we do not need to check for the resulting last colour
